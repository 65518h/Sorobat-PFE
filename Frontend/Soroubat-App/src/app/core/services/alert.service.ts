...
import { BehaviorSubject, Observable, interval, Subject, catchError, of, map } from 'rxjs';
...
export class AlertService {
  ...
  constructor() {
    this.loadReadStatus();
    this.loadRecentAlertsForNavbar(); // Chargement initial des alertes récentes
    this.startPolling();
  }
  ...
  // ✅ Pour la page complète des alertes
  getAllAlertsForPage(): Observable<Alert[]> {
  ...
  getSiteManagementAlerts(): Observable<Alert[]> {
    return this.http.get<Alert[]>(`${this.apiUrl}/site-management`);
  }

  getPurchaseRequestAlerts(): Observable<Alert[]> {
    return this.http.get<Alert[]>(`${this.apiUrl}/purchase-requests`);
  }

  getStockAlerts(): Observable<Alert[]> {
    return this.http.get<Alert[]>(`${this.apiUrl}/stock`);
  }

  getVehiculeAlerts(): Observable<Alert[]> {
    return this.http.get<Alert[]>(`${this.apiUrl}/vehicules`);
  }

  getGasoilAlerts(): Observable<Alert[]> {
    return this.http.get<Alert[]>(`${this.apiUrl}/gasoil`);
  }

  getTransferAlerts(): Observable<Alert[]> {
    return this.http.get<Alert[]>(`${this.apiUrl}/transfers`);
  }

  // ✅ Rafraîchir les alertes récentes pour le navbar
  refreshRecentAlerts(): void {
    this.loadRecentAlertsForNavbar();
  }

  // ✅ Rafraîchir toutes les alertes (pour la page complète)
  ...
  // ✅ CORRECTION: Interface UnreadCount complète
  ...
  private startPolling(): void {
    // Rafraîchir toutes les 3 minutes uniquement les alertes récentes
