...
export class AuthService {
  ...
  constructor() {
    ...
    // ✅ On ne fait pas de reconnexion automatique au démarrage
    // La connexion se fait uniquement via le formulaire
  }
  ...
  private clearOfflineSession(): void {
    ...
    console.log('🗑️ Session offline effacée');
  }
  ...
  async preloadDataForOffline(): Promise<void> {
    ...
    try {
      ...
      console.warn('⚠️ Échec préchargement stock:', error);
    }
    ...
    try {
      ...
      console.warn('⚠️ Échec préchargement pointages:', error);
    }
    ...
    try {
      ...
      console.warn('⚠️ Échec préchargement attendance:', error);
    }
    ...
    try {
      ...
      console.warn('⚠️ Échec préchargement tâches:', error);
    }
    ...
    try {
      ...
      console.warn('⚠️ Échec préchargement projet:', error);
    }
  ...
  login(email: string, password: string): Observable<LoginResponse> {
    ...
    if (!this.offlineSync.isOnline && this.hasValidOfflineSession()) {
      ...
      if (savedUser && savedUser.email === email) {
        ...
        this.toastr.warning(
          'Email différent de la session sauvegardée. Veuillez vous connecter en ligne.',
          'Connexion hors ligne'
        );
        return throwError(() => new Error('Email non trouvé dans la session offline'));
      }
    }
    
    // ✅ MODE ONLINE ou PAS DE SESSION OFFLINE
        ...
        catchError(error => {
          ...
          if ((error.status === 0 || error.status === 504) && this.hasValidOfflineSession()) {
            ...
            if (savedUser) {
              ...
              this.toastr.warning(
                'Serveur inaccessible. Utilisation du mode hors ligne avec votre dernière session.',
                'Mode dégradé',
                { timeOut: 5000, closeButton: true, progressBar: true }
              );
  ...
  logout(): void {
    ...
    if (this.isBrowser) {
      localStorage.removeItem('auth_token');
      localStorage.removeItem('remembered_credentials');
      // On garde user, offline_session_valid, last_login pour reconnexion offline
    }
    ...
    const message = !this.offlineSync.isOnline 
      ? 'Vous avez été déconnecté.\nVous pouvez vous reconnecter en mode hors ligne.'
      : 'Vous avez été déconnecté avec succès.';
    
    this.toastr.info(message, 'Déconnexion', { timeOut: 3000 });
  ...
  /**
   * ✅ Déconnexion complète (supprime aussi la session offline)
   */
  fullLogout(): void {
    ...
    this.toastr.info('Vous avez été déconnecté avec succès.', 'Déconnexion');
    this.router.navigate(['/auth/login']);
  }
  ...
  isAuthenticated(): boolean {
    ...
    // En mode hors ligne, on est authentifié si on a une session valide
    if (!this.offlineSync.isOnline && this.hasValidOfflineSession()) {
      return true;
    }
    
    // En mode en ligne, on a besoin d'un token valide
