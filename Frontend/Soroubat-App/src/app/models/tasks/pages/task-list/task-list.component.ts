...
export class TaskListComponent implements OnInit, OnDestroy {
  displayedColumns: string[] = [
    'jobNo', 'taskNo', 'description', 'dateDebut', 'dateFin', 
    'quantity', 'amount', 'status', 'progress', 'actions'
  ];
  ...
  private subscribeToAlertCounts(): void {
    this.alertsCounterService.counts$
      .pipe(takeUntil(this.destroy$))
      .subscribe(counts => {
        this.alertCount = counts.siteManagement;
        this.cdr.detectChanges();
      });
  }
    ...
    if (this.selectedStatusFilter !== 'all') {
      filteredTasks = filteredTasks.filter(task => {
        switch (this.selectedStatusFilter) {
          case 'en-cours':
            return !task.isBlocked && task.progressPct > 0 && task.progressPct < 100 && !this.isOverdue(task);
          case 'termine':
            return task.progressPct === 100;
          case 'bloque':
            return task.isBlocked === true;
          case 'retard':
            return this.isOverdue(task) && !task.isBlocked && task.progressPct < 100;
          default:
            return true;
        }
      });
  ...
  /**
   * Scroll jusqu'au tableau et filtre par statut
   */
  scrollToTableAndFilterByStatus(status: string): void {
    // 1. Appliquer le filtre
    this.filterByStatus(status);
    
    // 2. Forcer la détection des changements
    this.cdr.detectChanges();
    
    // 3. Scroll jusqu'au tableau avec un délai pour permettre le rendu
    setTimeout(() => {
      if (this.tasksTable && this.tasksTable.nativeElement) {
        ...
        // Ajouter une classe d'animation pour mettre en évidence le tableau
  ...
  /**
   * Retourne le texte du filtre pour le message
   */
  ...
  getInProgressTasksCount(): number {
    return this.tasks.filter(t => !t.isBlocked && t.progressPct > 0 && t.progressPct < 100 && !this.isOverdue(t)).length;
  }
  ...
  getQuantityShippedTooltip(task: JobTask): string {
    ...
    if (task.quantityShipped > task.initialQuantity) {
      ...
      tooltip += ` Dépassement: +${exceed} ${task.initialUoM}`;
