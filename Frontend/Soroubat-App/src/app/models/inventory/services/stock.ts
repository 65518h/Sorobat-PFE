...
export class StockService {
  ...
  getAllStock(): Observable<StockItem[]> {
    ...
    return this.http.get<StockItem[]>(this.apiUrl).pipe(
      ...
      tap(items => {
        ...
        console.log(`💾 ${items.length} articles de stock sauvegardés dans IndexedDB`);
      }),
      catchError(error => {
  ...
  /**
   * Récupère le stock avec filtres (avec cache pour les résultats filtrés)
   */
  getFilteredStock(filters?: StockFilter): Observable<StockItem[]> {
    ...
    return this.http.get<StockItem[]>(this.apiUrl, { params }).pipe(
      ...
      catchError(error => {
        ...
        // En cas d'erreur, filtrer depuis le cache
        return this.getFromCacheOffline().pipe(
          map(items => this.applyFiltersToItems(items, filters))
        );
      })
   ...
   */
  clearCache(): void {
    console.log('🗑️ Cache stock vidé');
