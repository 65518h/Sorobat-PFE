...
export class StockListComponent implements OnInit, OnDestroy {
  ...
  onScanSuccess(scannedCode: string): void {
    ...
    // 🔊 JOUER LE SON DE SCAN RÉUSSI (AudioService)
  ...
  private processScannedCode(scannedCode: string): void {
    ...
    setTimeout(() => {
      ...
      if (foundItem) {
        ...
        // 🔊 JOUER LE SON D'ERREUR (AudioService)
        this.audioService.playError();
        
        this.toastr.error(`Aucun article trouvé pour le code: ${scannedCode}`, 'Non trouvé', {
          positionClass: 'toast-top-right',
          timeOut: 4000
        });
        
        // Proposer de réessayer ou de fermer
        if (confirm(`L'article "${scannedCode}" n'existe pas dans le stock.\nVoulez-vous réessayer de scanner ?`)) {
          this.startScanner();
        } else {
          this.closeBarcodeScanner();
        }
