...
export interface GasoilHeader {
  ...
  totalQuantity?: number;  //  NOUVEAU - consommation max
}
...
export interface GasoilLine {
  ...
  vehicleNo?: string;
  quantity?: number;
  indexType?: string;      // 'Horaire' ou 'Kilometrage'
  valeurCompteur?: number; //  NOUVEAU - valeur unique du compteur (remplace hourIndex/kmIndex)
  projectNo?: string;
  driver?: string;         //  NOUVEAU - conducteur
  destination?: string;    //  NOUVEAU - destination
  observation?: string;    //  NOUVEAU - observation
  time?: string;           //  NOUVEAU - heure
