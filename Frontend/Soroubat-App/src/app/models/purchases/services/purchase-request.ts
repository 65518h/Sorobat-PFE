...
export class PurchaseRequestService {
  ...
  private normalizeStatus(status: string | undefined): string {
    ...
    const englishToFrenchMap: Record<string, string> = {
      'Open': 'Ouvert',
      'To Approve': 'Released',
      'Released': 'Released',
    ...
    const mappedStatus = englishToFrenchMap[cleanStatus];
    if (mappedStatus) {
      return mappedStatus;
    }
  ...
  private decodePurchaseRequest(request: any): PurchaseRequest {
    ...
    return {
      ...
      status: this.normalizeStatus(request.statut || request.status),
  ...
  update(id: string, updates: UpdatePurchaseRequest): Observable<void> {
    ...
    console.log('📡 Appel API PATCH:', `${this.API_URL}/${id}`, updates);
  ...
  updateStatus(id: string, status: string): Observable<void> {
    ...
    return this.update(id, { status: apiStatus });
  }
   ...
   */
  approve(id: string): Observable<PurchaseRequest> {
    if (!id) {
      return throwError(() => new Error('ID requis pour approuver la demande'));
    }
    
    console.log('📡 Appel API approve:', `${this.API_URL}/${id}/approve`);
    
    return this.http.post<any>(`${this.API_URL}/${id}/approve`, {}).pipe(
      timeout(this.TIMEOUT_MS),
      map(request => this.decodePurchaseRequest(request)),
      tap(request => {
        this.cacheService.invalidatePurchaseRequests();
  ...
  reject(id: string, reason?: string): Observable<PurchaseRequest> {
    if (!id) {
      return throwError(() => new Error('ID requis pour refuser la demande'));
    }
    
    const body = reason ? { reason } : {};
    console.log('📡 Appel API reject:', `${this.API_URL}/${id}/reject`, body);
    
    return this.http.post<any>(`${this.API_URL}/${id}/reject`, body).pipe(
      timeout(this.TIMEOUT_MS),
      map(request => this.decodePurchaseRequest(request)),
      tap(request => {
        this.cacheService.invalidatePurchaseRequests();
   ...
   * Annule une demande
   */
  cancel(id: string, reason?: string): Observable<PurchaseRequest> {
    if (!id) {
      return throwError(() => new Error('ID requis pour annuler la demande'));
    }
    
    const body = reason ? { reason } : {};
    console.log('📡 Appel API cancel:', `${this.API_URL}/${id}/cancel`, body);
    
    return this.http.post<any>(`${this.API_URL}/${id}/cancel`, body).pipe(
      timeout(this.TIMEOUT_MS),
      map(request => this.decodePurchaseRequest(request)),
      tap(request => {
        this.cacheService.invalidatePurchaseRequests();
   ...
   * Archive une demande
   */
  archive(id: string): Observable<PurchaseRequest> {
    if (!id) {
      return throwError(() => new Error('ID requis pour archiver la demande'));
    }
    
    console.log('📡 Appel API archive:', `${this.API_URL}/${id}/archive`);
    
    return this.http.post<any>(`${this.API_URL}/${id}/archive`, {}).pipe(
      timeout(this.TIMEOUT_MS),
      map(request => this.decodePurchaseRequest(request)),
      tap(request => {
        this.cacheService.invalidatePurchaseRequests();
  ...
  getByStatus(status: string): Observable<PurchaseRequest[]> {
    return this.getAll({ status });
  }

  getByRequester(requesterId: string): Observable<PurchaseRequest[]> {
    return this.getAll({ requesterId });
  }
  ...
  getPendingApprovals(): Observable<PurchaseRequest[]> {
    return this.getAll({ status: 'To Approve' });
  }

  getOpenRequests(): Observable<PurchaseRequest[]> {
    return this.getAll({ status: 'Ouvert' });
  }
}