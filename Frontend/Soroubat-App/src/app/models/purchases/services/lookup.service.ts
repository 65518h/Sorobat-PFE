// src/app/models/purchases/services/lookup.service.ts
...
getVehicules(): Observable<Vehicule[]> {
  const url = `${this.BASE_URL}/VehiculePointage`;
  ...
  return this.http.get<any[]>(url).pipe(
    ...
    switchMap(pointages => {
      ...
      const detailRequests = pointages.map(pointage => 
        this.http.get<any>(`${this.BASE_URL}/VehiculePointage/${pointage.id}`).pipe(
          catchError(error => {
