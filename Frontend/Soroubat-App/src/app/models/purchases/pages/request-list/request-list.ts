...
export class RequestListComponent implements OnInit, OnDestroy {
  ...
  // Données master pour les statistiques (non filtrées)
  masterRequests: PurchaseRequest[] = [];
  ...
  // Statistiques globales (basées sur masterRequests, non filtrées)
  statsData = [
    { icon: 'description', value: 0, label: 'Total demandes', gradient: 'linear-gradient(135deg, #6366f1, #8b5cf6)', trend: 0 },
    { icon: 'pending', value: 0, label: 'Ouvert', gradient: 'linear-gradient(135deg, #f59e0b, #d97706)', trend: 0 },
    { icon: 'play_circle', value: 0, label: 'A approuver', gradient: 'linear-gradient(135deg, #3b82f6, #2563eb)', trend: 0 },
  ...
  async preloadRecentRequestDetails(requests: PurchaseRequest[]): Promise<void> {
    if (this.preloadCompleted) {
      console.log('Prechargement deja effectue');
      return;
    }
    
    if (!this.offlineSync.isOnline) {
      console.log('Hors ligne - Pas de prechargement');
      return;
    }
    
    if (this.isPreloading) {
      console.log('Prechargement deja en cours...');
      return;
    }
    ...
    if (requestsToPreload.length === 0) {
      console.log('Aucune demande a precharger');
    ...
    for (let i = 0; i < requestsToPreload.length; i += batchSize) {
      ...
      const promises = batch.map(async (request) => {
        if (request.id) {
          try {
            ...
            successCount++;
            console.log(`Precharge: ${request.no}`);
          } catch (err) {
            console.warn(`Echec prechargement ${request.no}:`, err);
            failCount++;
          }
        }
      });
      ...
      console.log(`Batch termine (${this.preloadCurrent}/${this.preloadTotal})`);
    }
    
    console.log(`Prechargement termine: ${successCount} succes, ${failCount} echecs`);
  ...
  navigateToAlerts(): void {
    ...
    this.alertService.getPurchaseRequestAlerts().pipe(
      ...
      next: (alerts) => {
        ...
        console.log(`Alertes demandes d'achat recuperees: ${alertCount}`);
        
        if (alertCount > 0) {
          this.soundService.playDefaultSound();
          this.toastr.info(
            `${alertCount} alerte${alertCount > 1 ? 's' : ''} dans les demandes d'achat`,
            'Alertes disponibles',
            {
      ...
      error: (error) => {
        ...
        this.toastr.error('Impossible de verifier les alertes', 'Erreur');
  ...
  private applyFilterByStat(statIndex: number): void {
    ...
    // Reinitialiser les filtres existants
    ...
    switch (statIndex) {
      ...
      case 1: // Ouvert
        ...
        break;
      case 2: // A approuver (Released / En cours)
    ...
    // Appliquer le filtre de statut sur les données (sans affecter les stats globales)
    if (statusFilters.length === 0) {
    ...
    this.currentPage = 1;
    // Ne pas appeler updateStats ici - les stats restent globales
  ...
  loadRequests(): void {
    ...
    this.purchaseRequestService.getAll()
      ...
      .subscribe({
        next: (requests) => {
          console.log('Demandes recues:', requests?.length || 0);
          this.masterRequests = requests || [];
  ...
  // Mettre à jour les statistiques globales (basées sur masterRequests)
  updateGlobalStats(): void {
    this.statsData[0].value = this.masterRequests.length;
    this.statsData[1].value = this.masterRequests.filter(r => r.statut === 'Ouvert' || r.statut === 'Open').length;
    this.statsData[2].value = this.masterRequests.filter(r => r.statut === 'Released' || r.statut === 'En cours').length;
    this.statsData[3].value = this.masterRequests.filter(r => r.statut === 'approved' || r.statut === 'Approved').length;
    this.statsData[4].value = this.masterRequests.filter(r => 
    ...
    console.log('Statistiques globales:', {
      total: this.statsData[0].value,
      open: this.statsData[1].value,
      toApprove: this.statsData[2].value,
      approved: this.statsData[3].value,
      completed: this.statsData[4].value
    });
  }
  ...
  applyLocalFilters(): void {
    ...
    // Appliquer d'abord le filtre de statut si present
  ...
  createNewRequest(): void {
    if (this.isReadOnly) {
      this.toastr.warning('Creation de demande indisponible en mode hors ligne', 'Action indisponible');
      return;
    }
  ...
  editRequest(request: PurchaseRequest): void {
    ...
    if (!this.canEdit(request)) {
      this.notificationService.showWarning('Cette demande ne peut pas etre modifiee');
      return;
    }
  ...
  deleteRequest(request: PurchaseRequest): void {
    ...
    if (!this.canDelete(request)) {
      this.notificationService.showWarning('Seules les demandes ouvertes peuvent etre supprimees');
      return;
    }
    
    const dialogData: ConfirmationDialogData = {
      title: 'Supprimer la demande',
      message: `Etes-vous sûr de vouloir supprimer la demande ${request.no} ?\n\nCette action est irreversible.`,
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      ...
      this.purchaseRequestService.delete(id).pipe(
        ...
        next: () => {
          // Mettre à jour masterRequests et requests
          const masterIndex = this.masterRequests.findIndex(r => r.id === id);
          if (masterIndex !== -1) {
            this.masterRequests.splice(masterIndex, 1);
          }
          ...
          this.updateGlobalStats();
          this.cdr.detectChanges();
          this.notificationService.showSuccess('Demande supprimee avec succes');
          this.toastr.success(`Demande ${request.no} supprimee`, 'Suppression reussie');
        },
        error: (error) => {
          console.error('Erreur suppression', error);
          this.notificationService.showError('Impossible de supprimer la demande');
          this.toastr.error(`Erreur lors de la suppression de la demande ${request.no}`, 'Echec');
        }
      });
    });
  }
  ...
  generateOrders(): void {
    if (this.isReadOnly) {
      this.toastr.warning('Generation de commande indisponible en mode hors ligne', 'Action indisponible');
      return;
    }
    ...
    console.log('Demandes selectionnees (IDs):', selectedIds);
    
    this.notificationService.showConfirmation({
      title: 'Generation de commandes',
      message: `Generer les commandes fournisseur pour ${this.selectedRequests.size} demande(s) ?`,
      confirmText: 'Generer',
      cancelText: 'Annuler'
    }).subscribe(confirmed => {
      if (confirmed) {
        this.notificationService.showSuccess('Commandes generees avec succes');
        this.selectedRequests.clear();
        this.loadRequests();
      }
    });
  ...
  updateStats(): void {
    // Ne rien faire - les stats sont maintenant gérées par updateGlobalStats
    // Cette méthode est conservée pour éviter les erreurs d'appel
  }
  ...
  releaseRequest(request: PurchaseRequest): void {
    ...
    if (!this.canRelease(request)) {
      this.notificationService.showWarning(`Cette demande ne peut pas etre lancee`);
      return;
    }
    
    const dialogData: ConfirmationDialogData = {
      title: 'Lancer la demande',
      message: `Confirmez-vous le lancement de la demande ${request.no} ?\n\nUne fois lancee, la demande sera envoyee pour approbation et ne pourra plus etre modifiee.`,
      confirmText: 'Lancer',
      cancelText: 'Annuler',
      ...
      this.purchaseRequestService.submitToApprove(id).pipe(
        ...
        next: (updatedRequest) => {
          if (updatedRequest) {
            ...
            const masterIndex = this.masterRequests.findIndex(r => r.id === id);
            if (masterIndex !== -1) {
              this.masterRequests[masterIndex] = updatedRequest;
            }
          ...
          this.toastr.success(`Demande ${request.no} lancee avec succes`, 'Demande envoyee');
          this.soundService.playNotificationSound();
        },
        error: (error) => {
          console.error('Erreur lancement', error);
          this.notificationService.showError('Impossible de lancer la demande');
          this.toastr.error(`Erreur lors du lancement de la demande ${request.no}`, 'Echec');
        }
      });
    });
  }
  ...
  public getCurrentStatFilterLabel(): string {
    ...
    const labels: Record<string, string> = {
      'Ouvert': 'Ouvert',
      'Released': 'A approuver',
      'approved': 'Approuvees',
      'Totallement Pris En Charge': 'Traites'
    };
