...
export class RequestForm implements OnInit, OnDestroy {
    ...
    this.projectService.getMyProject().pipe(
      ...
      next: (project: MyProject | null) => {
        if (project) {
          ...
          console.warn('⚠️ Aucun projet associé');
    ...
    this.purchaseRequestService.getById(id).pipe(
      ...
      next: (request: PurchaseRequest | null) => {
        if (request) {
          ...
          console.log('📋 Statut de la demande:', request.status);
          ...
          const isOpen = request.status === 'Ouvert' || request.status === 'Open';
  ...
  private populateForm(request: PurchaseRequest): void {
    ...
    this.headerForm.patchValue({
      ...
      dueDate: request.dueDate && request.dueDate !== '0001-01-01' ? new Date(request.dueDate) : null
    });
  ...
  createLineForm(line?: PurchaseRequestLine): FormGroup {
    ...
    return this.fb.group({
      ...
      description: [{ value: line?.description || '', disabled: true }],
  ...
  clearAllLines(): void {
    ...
    const confirmed = confirm(`Supprimer tous les ${this.linesArray.length} articles ?`);
    if (!confirmed) return;
  ...
  createHeaderAndContinue(): void {
    ...
    const requestData: any = {
      ...
      orderDate: this.formatDateForApi(formValues.orderDate),
      dueDate: this.formatDateForApi(formValues.dueDate)
    };
  ...
  private updateAllLines(): Observable<any> {
    ...
    for (let i = 0; i < this.linesArray.length; i++) {
      ...
      const engin = lineForm.get('engin')?.value;
      ...
      if (lineId) {
        ...
        if (engin !== undefined && engin !== null && engin !== '') updateLineData.engin = engin;
...
openArticleDialog(lineIndex: number): void {
  ...
  // Créer un backdrop personnalisé
  ...
  // Ajouter manuellement un style au backdrop
  setTimeout(() => {
  ...
  private processScannedCode(scannedCode: string, lineIndex: number): void {
    ...
    setTimeout(() => {
      ...
      if (foundItem) {
        ...
        this.notificationService.showSuccess(`Article "${foundItem.displayName}" trouvé via code-barres !`);
        this.closeBarcodeScanner();
      } else {
        // 🔊 JOUER LE SON D'ERREUR
        this.audioService.playError();
        
        this.notificationService.showWarning(`Aucun article trouvé pour le code: ${scannedCode}`);
        
        if (confirm(`L'article "${scannedCode}" n'existe pas. Voulez-vous le rechercher manuellement ?`)) {
          this.closeBarcodeScanner();
  ...
  cancel(): void {
    if (this.linesArray && (this.linesArray.length > 0 || this.headerForm.dirty)) {
