...
export interface AttendanceHeader {
  id?: string;           // ← CHANGÉ : GUID au lieu de no
  no?: string;           // ← AJOUTÉ : Numéro du document (pour affichage)
  month: string;
  year: number;
  jobNo: string;
  totalStaff?: number;
  attendanceRate?: number;
   isOffline?: boolean;  // ✅ AJOUTER CETTE PROPRIÉTÉ
  employeeAttendanceLines?: AttendanceLine[];
  lines?: AttendanceLine[];
}

export interface AttendanceLine {
  id?: string;           // ← CHANGÉ : GUID
  jobNo?: string;
  documentNo?: string;   // ← CHANGÉ : documentNo au lieu de documentNo
  employeeNo: string;
  employeeName: string;
  assignment?: string;
...
// Codes de pointage avec leurs significations
