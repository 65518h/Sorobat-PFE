...
export class AttendanceFormComponent implements OnInit, OnDestroy {
  ...
  //  Méthode pour obtenir les initiales de l'employé
  ...
  selectEmployeeForCurrentLine(employee: Employee): void {
    ...
    if (assignment !== (employee.fonction || '')) {
      console.log(` Fonction tronquée: "${employee.fonction}" -> "${assignment}"`);
    }
  ...
  //  Modifiez la méthode deleteLine
  deleteLine(line: AttendanceLine): void {
    const isTemp = (line as any).__isTemp;
    const employeeName = line.employeeName || 'cet employé';
    
    // Dialogue de confirmation personnalisé
    const dialogData: ConfirmDialogData = {
      title: 'Supprimer un employé',
      message: `Êtes-vous sûr de vouloir supprimer "${employeeName}" de ce pointage ?\n\n⚠️ Cette action est irréversible et supprimera toutes les présences associées.`,
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      ...
      if (isTemp) {
        ...
        this.ngZone.run(() => {
          this.toastr.success(`${employeeName} supprimé du pointage`);
        });
      } else {
        this.loading = true;
        this.cdr.detectChanges();
  ...
  saveAllChanges(): void {
    ...
    if (this.isEditMode && this.attendanceForm.dirty) {
      ...
      if (this.attendanceForm.get('month')?.dirty) {
        headerData.month = formValue.month;
      }
      if (this.attendanceForm.get('year')?.dirty) {
        headerData.year = formValue.year;
      }
      
      if (Object.keys(headerData).length > 0) {
        console.log(' Mise à jour header (champs modifiés uniquement):', headerData);
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode) {
      ...
      if (this.attendanceForm.get('month')?.dirty) {
        updateData.month = formValue.month;
      }
      if (this.attendanceForm.get('year')?.dirty) {
        updateData.year = formValue.year;
      }
          ...
          error: (error) => {
            ...
            this.ngZone.run(() => {
              if (error.status === 409) {
                ...
                this.toastr.error(error.error?.message || 'Erreur lors de la création');
              }
            });
            this.loading = false;
            this.cdr.detectChanges();
  ...
  //  Modifiez la méthode deleteAttendance
  ...
  const dialogData: ConfirmDialogData = {
    ...
    message: `Êtes-vous sûr de vouloir supprimer le pointage de ${monthYear || 'cette période'} ?\n\n⚠️ Cette action est irréversible et supprimera également tous les employés associés.`,
    confirmText: 'Supprimer',
    cancelText: 'Annuler',
    confirmColor: 'warn'
  };
