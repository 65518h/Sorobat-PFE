...
import { Alert, DOMAINS } from '../../models/alerts.model';
...
// ✅ Nouvelle méthode pour initialiser le cache depuis l'API
private initializeCacheFromApi(cacheKey: string, limitPerDomain: number): void {
  ...
  private loadAlertsFromApi(cacheKey: string, limitPerDomain: number): void {
    this.alertsService.getAllAlertsByDomain(limitPerDomain).pipe(
      takeUntil(this.destroy$),
      catchError(error => {
        console.error('❌ Erreur chargement alertes:', error);
        this.loading = false;
        this.notificationService.showError('Erreur chargement des alertes');
        this.cdr.detectChanges();
        return of(null);
      })
    ).subscribe({
  ...
  private hasAlertsChanged(newAlerts: any): boolean {
    const currentTotal = this.siteManagementAlerts.length + this.purchaseAlerts.length + 
                         this.transferAlerts.length + this.stockAlerts.length + 
                         this.vehiculeAlerts.length + this.gasoilAlerts.length;
    const newTotal = (newAlerts.siteManagement?.length || 0) + (newAlerts.purchaseRequests?.length || 0) +
                     (newAlerts.transfers?.length || 0) + (newAlerts.stock?.length || 0) +
                     (newAlerts.vehicules?.length || 0) + (newAlerts.gasoil?.length || 0);
    return currentTotal !== newTotal;
  }
  
  private applyAlertsData(alerts: any): void {
    this.siteManagementAlerts = alerts.siteManagement || [];
    ...
    console.log('✅ Alertes chargées:', {
      siteManagement: this.siteManagementAlerts.length,
  ...
  getFilteredDomainAlerts(domainKey: string): Alert[] {
    ...
    switch(domainKey) {
      case 'siteManagement':
        alerts = [...this.siteManagementAlerts];
        break;
  ...
  getFilteredDomainsCount(): number {
    let count = 0;
    if (this.getFilteredDomainAlerts('siteManagement').length > 0) count++;
  ...
  private getAllAlertsForCurrentDomain(): Alert[] {
    ...
    switch(this.selectedDomain) {
      case 'siteManagement': return [...this.siteManagementAlerts];
  ...
  applyFilters(): void {
    ...
    if (this.selectedDomain === 'all' || this.selectedDomain === 'siteManagement') {
      total += this.getFilteredDomainAlerts('siteManagement').length;
    }
  ...
  private updateAlertInLists(alertId: string): void {
    this.siteManagementAlerts = this.updateSingleAlert(this.siteManagementAlerts, alertId);
  ...
  getTotalAlertsCount(): number {
    return this.siteManagementAlerts.length + this.purchaseAlerts.length + 
           this.transferAlerts.length + this.stockAlerts.length + 
           this.vehiculeAlerts.length + this.gasoilAlerts.length;
  }
  ...
  // alerts-page.ts - Ajoutez cette méthode

private initializeCacheIfNeeded(): void {
  // Vérifier si le cache est vide
  this.cacheService.getFromCache('alerts-all-domains').then(cachedData => {
    if (!cachedData && !this.isReadOnly) {
      // Cache vide et on est en ligne -> charger et mettre en cache
      console.log('🔄 Initialisation du cache des alertes...');
      const limitPerDomain = 50;
      const cacheKey = 'alerts-all-domains';
      ...
      this.alertsService.getAllAlertsByDomain(limitPerDomain).pipe(
        ...
        next: (alerts) => {
          if (alerts) {
            this.applyAlertsData(alerts);
            this.cacheService.saveToCache(cacheKey, alerts);
            console.log('💾 Cache des alertes initialisé avec succès');
          }
          this.loading = false;
          this.cdr.detectChanges();
        },
