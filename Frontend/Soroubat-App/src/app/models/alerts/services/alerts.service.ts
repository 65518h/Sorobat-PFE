...
import { BehaviorSubject, Observable, interval, catchError, of, forkJoin, map } from 'rxjs';
...
export class AlertsService {
  ...
  // Cache des alertes par domaine avec pagination
  ...
  /**
   * Décode les chaînes encodées en XML
   * Transforme "Fully_x0020_Supported" en "Fully Supported"
   */
  ...
  /**
   * Décode une alerte complète
   */
  private decodeAlert(alert: Alert): Alert {
    ...
    return {
      ...
      status: alert.status ? this.decodeXmlString(alert.status) : alert.status  // ← Correction ici
    };
  }

  /**
   * Décode un tableau d'alertes
   */
  private decodeAlerts(alerts: Alert[]): Alert[] {
  ...
  getPaginatedAlerts(page: number = 1, pageSize: number = 20): Observable<PaginatedResponse<Alert>> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('pageSize', pageSize.toString());
    
    return this.http.get<PaginatedResponse<Alert>>(`${this.apiUrl}/all/paginated`, { params }).pipe(
      map(response => ({
        ...response,
        data: this.decodeAlerts(response.data)
      })),
      catchError(error => {
        console.error('Erreur chargement alertes paginées:', error);
        return of({ data: [], total: 0, page: 1, pageSize: 20, totalPages: 0 });
      })
    );
  }

  // ==================== Récupération par domaine avec cache ====================
  
  getSiteManagementAlerts(forceRefresh: boolean = false): Observable<Alert[]> {
    return this.getCachedOrFetch('siteManagement', () => 
      this.http.get<Alert[]>(`${this.apiUrl}/site-management`).pipe(
  ...
  // Récupérer uniquement les compteurs (léger)
  getAlertsCount(): Observable<{ [key: string]: number }> {
    return this.http.get<{ [key: string]: number }>(`${this.apiUrl}/counts`).pipe(
      catchError(error => {
  ...
  // Récupérer les alertes récentes (limité)
  getRecentAlerts(limit: number = 10): Observable<Alert[]> {
    const params = new HttpParams().set('limit', limit.toString());
    return this.http.get<Alert[]>(`${this.apiUrl}/recent`, { params }).pipe(
      map(alerts => this.decodeAlerts(alerts)),
      catchError(error => {
  ...
  // Récupérer toutes les alertes par domaine en parallèle avec limite
  getAllAlertsByDomain(limitPerDomain?: number): Observable<{ [key: string]: Alert[] }> {
    ...
    return forkJoin({
      siteManagement: this.http.get<Alert[]>(`${this.apiUrl}/site-management`, options).pipe(
        map(alerts => this.decodeAlerts(alerts)),
        catchError(() => [])
      ),
  ...
  refreshAllAlerts(limitPerDomain?: number): void {
    this.getAllAlertsByDomain(limitPerDomain).subscribe(alerts => {
      const allAlerts = [
        ...alerts.siteManagement,
