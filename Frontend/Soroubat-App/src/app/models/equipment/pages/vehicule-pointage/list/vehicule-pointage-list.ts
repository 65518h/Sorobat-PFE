...
export class VehiculePointageList implements OnInit, OnDestroy {
  ...
  /**
   * Retourne le total des heures pour un pointage
   */
  getTotalHours(pointage: VehiculePointageHeader): number {
    return pointage.totalHours || 0;
  }
  
  /**
   * Retourne la distance totale pour un pointage
   */
  getTotalDistance(pointage: VehiculePointageHeader): number {
    return pointage.totalDistance || 0;
  }
  
  /**
   * Retourne le carburant total pour un pointage
   */
  getTotalFuel(pointage: VehiculePointageHeader): number {
    return pointage.totalFuel || 0;
  }
  
  /**
   * Retourne le nombre de véhicules pour un pointage
   */
  getVehiculesCount(pointage: VehiculePointageHeader): number {
    return pointage.totalVehicules || 0;
  }
  ...
  validatePointage(pointage: VehiculePointageHeader): void {
    ...
    if (confirm(`Valider le pointage ${pointage.documentNo} ?\n\nAprès validation, vous ne pourrez plus modifier ce pointage.`)) {
      this.loading = true;
      this.cdr.detectChanges();
  ...
  deletePointage(pointage: VehiculePointageHeader): void {
    ...
    if (confirm(`Supprimer définitivement le pointage ${pointage.documentNo} ?\n\nCette action est irréversible.`)) {
      this.vehiculePointageService.deleteHeader(pointage.id!).subscribe({
        next: () => {
